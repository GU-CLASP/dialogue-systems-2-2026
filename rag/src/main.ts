...
program
  .command("queryRerank")
  .description("Query with cosine similarity vs. keyword-overlap re-ranking, side by side.")
  .argument("<collection>", "collection name")
  .argument("<query>", "text of the query")
  .action(async (collection, query) => {
    ...
    const queryWords = query.toLowerCase().split(/\s+/); // first splitting query into words

    const scored = results.points.map((p) => { // we process every search result
      const text = ((p.payload as any)?.text ?? "").toLowerCase(); // extracting it
      const keywordHits = queryWords.filter((w) => text.includes(w)).length; // we count the keyword matches
      // then we create the result object but richer this time
      return {
        id: p.id,
        text: (p.payload as any)?.text ?? "",
        cosineScore: p.score,
        keywordHits,
        combinedScore: p.score + keywordHits * 0.05,
      };
    });

    console.log("\n--- Top 3 results by raw cosine similarity ---");
    [...scored]// copy of the array
      .sort((a, b) => b.cosineScore - a.cosineScore) // it sorts by cosine score
      .slice(0, 3) // we take only three items
      .forEach((r, i) => { // r is the result object and i is index
        console.log(`\n${i + 1} ) cosine = ${r.cosineScore.toFixed(4)} and keywordHits = ${r.keywordHits}`);
        console.log(r.text.slice(0, 200) + "..."); // takes only the first 200 characters
      });

    console.log("\n\n-- After reranking, Top 3 -- (both metrics)");
    [...scored]
      .sort((a, b) => b.combinedScore - a.combinedScore)
      .slice(0, 3)
      .forEach((r, i) => {
        console.log(`\n${i + 1} ) combined=${r.combinedScore.toFixed(4)} and cosine=${r.cosineScore.toFixed(4)} and keywordHits=${r.keywordHits}`);
...
program
  .command("queryLimits")
  .description("Compare retrieved context at limit=1, 3, and 8 for the same query.")
  .argument("<collection>", "collection name")
  .argument("<query>", "text of the query")
  .action(async (collection, query) => {
    const embedding = await embed(query);

    for (const limit of [1, 3, 8]) { // for each limit
      const results = await client.query(collection, { // we get the results
        with_payload: true,
        query: embedding,
        limit,
      });

      const combinedText = results.points // then we build one big context string
        .map((p) => (p.payload as any)?.text ?? "")
        .join("\n\n");

      console.log(`\n----------- limit = ${limit}`);
      console.log(`Total chars in retrievedContext => ${combinedText.length}`); // the context size
      console.log(`Chunk sources => ${results.points.map((p) => (p.payload as any)?.source).join(", ")}`); // for showing chunk sources
      console.log(`Lowest score included => ${results.points[results.points.length - 1]?.score.toFixed(4)}`);
    }
  });
