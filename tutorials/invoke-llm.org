...
#+begin_quote
[!IMPORTANT]
The videos are from 2025 and therefore there might be some differences from newer versions of SpeechState.
#+end_quote
...
own machine. You can connect to mltgpu's (or mltgpu-2's) ollama with
this command:
...
ollama API. [[https://docs.ollama.com/api/introduction][This document]] describes ollama's Application Programming
Interface (API). 
...
#+begin_quote
[!IMPORTANT]
You can use starter code from this repository to test this out.
#+end_quote
...
* Exercise 2
Instead of using ~fetch~, use the OpenAI library and modify your code, so that it uses Chat Completion API. If you have already used Groq, you can use it via this API too.
...
Starter code for creating OpenAI API client instance, for ollama (goes
on the top level, outside of the state machine).
...
The chat completion code (`await openai.chat.completions.create({...`) will go into the actor definition.
